    ...
    public function notifyUrl(Request $request)
    {
        try {
            ...
            if ($data['PAY_STATUS'] === 'S' && $data['RCODE'] === '0000') {
                ...
                Log::info('P99Pay notify payment successful', [
                    'orderSN' => $orderSN,
                    'amount' => $data['AMOUNT'],
                    'trade_no' => $data['RRN']
                ]);

                // 根據文檔要求返回 RRN|PAY_STATUS
                return $data['RRN'] . '|' . $data['PAY_STATUS'];
            }

            // 處理失敗狀態
            Log::warning('P99Pay notify payment failed', [
                'orderSN' => $orderSN,
                'status' => $data['PAY_STATUS'],
                'rcode' => $data['RCODE'],
                'message' => $data['RMSG_CHI'] ?? '支付失敗'
            ]);

            // 即使支付失敗也返回 RRN|PAY_STATUS，讓 P99 停止通知
            return $data['RRN'] . '|' . $data['PAY_STATUS'];

        } catch (\Exception $e) {
            Log::error('P99Pay notify processing error:', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            return response('處理失敗', 500);
        }
    }
