...
<div class="row">
    <div class="col-md-6 offset-md-3">
        <div class="card border-primary border">
            <div class="card-body">
                <h5 class="card-title text-primary text-center">{{ __('hyper.qrpay_order_expiration_date') }} {{ config('app.order_expire_date') }} {{ __('hyper.qrpay_expiration_date') }}</h5>
        ...
        var getting = {
            ...
            success:function(res) {
                if (res.code == 400001) {
                    ...
                    $.NotificationApp.send("{{ __('hyper.qrpay_notice') }}","{{ __('prompt.order_pay_timeout') }}","bottom-right","rgba(0,0,0,0.2)","warning");
                    setTimeout("window.location.href ='/'",3000);
                }
                if (res.code == 200) {
                    window.clearTimeout(timer);
                    $.NotificationApp.send("{{ __('hyper.qrpay_notice') }}","{{ __('prompt.payment_successful') }}","bottom-right","rgba(0,0,0,0.2)","success");
                    setTimeout("window.location.href ='{{ url('searchOrderById', ['order_id' => $orderid]) }}'",3000);
                }
            }
        };
